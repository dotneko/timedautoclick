...
# Define the screen area to capture: (left, top, right, bottom)

QUEUE_CAPTURE_BOX = (72, 473, 293, 511)
#QUEUE_CAPTURE_BOX = (92, 381, 200, 415)

# Output CSV filename
CSV_FILE = "sp_queue_log.csv"
...
def capture_queue_num(bbox):
    """Captures a specific screen area, extracts text, and appends it to a CSV."""
    MAX_TRIES = 100
    count = 0
    while count < MAX_TRIES:
        try:
            # 1. Capture the specified area of the screen
            screenshot = ImageGrab.grab(bbox=bbox)

            # 2. Configure Tesseract for digits only
            # -c tessedit_char_whitelist=0123456789 restricts characters
            # --psm 6 assumes a single uniform block of text (use --psm 11 if numbers are scattered)
            custom_config = r'--psm 6 -c tessedit_char_whitelist=0123456789'
            extracted_text = pytesseract.image_to_string(screenshot, config=custom_config).strip()
        except Exception as e:
            print(f"An error occurred: {e}")

        try:
            queue_num = int(extracted_text)
        except ValueError:
            count += 1
        else:
            return queue_num
...
#     # 4. Check if file exists to determine if we need a header
#     file_exists = os.path.exists(CSV_FILE)

#     # 5. Open CSV and append data using built-in csv module
#     # newline='' prevents blank rows on Windows systems
#     with open(CSV_FILE, mode="a", newline="", encoding="utf-8") as file:
#         writer = csv.writer(file)

#         # Write header if it is a brand new file
#         if not file_exists:
#             writer.writerow(["Timestamp", "Extracted_Text"])

#         # Write the data row
#         writer.writerow([current_time, extracted_text])

#     print(f"[{current_time}] Successfully logged data.")
#     print(f"Text snippet: {repr(extracted_text[:30])}...")
...
def main():
    ...
    queue_num = capture_queue_num(QUEUE_CAPTURE_BOX)
    if queue_num:
        print(f"Offset: {args.offset}ms | Queue: {queue_num}")
    else:
        print("No queue num obtained")
